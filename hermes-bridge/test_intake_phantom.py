...
CASES = [
    ...
    ("hollow stub + definitive no-message -> phantom (suppress)",
     HOLLOW, NO_MSG_RESP, True),
    ("hollow stub + TRANSPORT error -> NOT phantom (still alert)",
     HOLLOW, TRANSPORT_ERR, False),
    ("hollow stub + ingest OK -> NOT phantom (it got recovered)",
     HOLLOW, OK_RESP, False),
    ("real lead w/ body + no-message resp -> NOT phantom (has text+ts)",
     REAL, NO_MSG_RESP, False),
    ("real lead w/ body + transport error -> NOT phantom",
     REAL, TRANSPORT_ERR, False),
    ("hollow but has a name -> NOT phantom (some identity present)",
     dict(HOLLOW, name="Maybe Someone"), NO_MSG_RESP, False),
    ("hollow but has body text -> NOT phantom (a message exists)",
     dict(HOLLOW, body="hello?"), NO_MSG_RESP, False),
    ("real timestamp (recent age) + empty name/body -> NOT phantom",
     dict(HOLLOW, age_days=2.0), NO_MSG_RESP, False),
    ("age_days None -> treated as 0, NOT phantom",
