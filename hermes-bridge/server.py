...
def hermes_analyze_lead(customer_id, history, facts, message_count=0,
    ...
    q = (
        ...
        "PATTERN RECOGNITION RUBRIC — apply in order, FIRST MATCH WINS:\n\n"
        "  CLOSE (score 0):\n"
        "  - Supplier / vendor collab pitch (introducing themselves as "
        "providing a service, 'collaboration', 'partnership offer', 'we "
        "offer', 'our agency') → NOT a customer, close lead.\n"
        "  - Spam / competitor probe: yacht names not in our catalog, "
        "impossible dates (e.g. years out + multiple inconsistent dates), "
        "info contradicts itself, customer-supplied data has no plausible "
        "logic → close.\n"
        "  - Explicit 'not interested' / 'found another operator' / "
        "'cancelled trip' / 'no longer needed' → close.\n"
        "  - Repeated price rejection (3+ times) with no flexibility shown "
        "→ close.\n"
        "  - Ghosted 14+ days after a clear rejection signal → close.\n\n"
        "  LOW PRIORITY (KEEP_OPEN, score 10-30):\n"
        "  - B2B / partnership inquiry (talks about 'partner', 'own boat', "
        "'manage', 'broker', 'agency', 'long-term', '6+ months', requests "
        "for management/call with owner/captain). Operator priority: "
        "always BELOW retail customers; not a sales fire-drill. Use "
        "score 10-25.\n\n"
        "  TIRE-KICKER (KEEP_OPEN, score 30-45):\n"
        "  - Multi-vendor blast: copy-paste shopping template, asks for "
        "5+ yachts at once, 'best rate', 'best price', generic format "
        "suggesting message sent to many operators in parallel.\n"
        "  - Price-sensitive but not engaged: keeps asking for cheaper, "
        "no commitment signals.\n\n"
        "  WAITING (KEEP_OPEN, score 45-65) — explain WHY they went silent:\n"
        "  - Quoted-then-silent: we sent a price/options, customer silent "
        "12h+, no rejection. Suggest: 'have you given up on booking a "
        "yacht?' nudge. Reasoning MUST identify the likely reason: "
        "price hesitation? Comparing with others? Lost interest? Mention it.\n"
        "  - First-time inquiry, hasn't fully replied yet → 50-65.\n\n"
        "  ACTIVE NEGOTIATION (KEEP_OPEN, score 65-85):\n"
        "  - Asked for payment link → we sent → no reply (>2h): score "
        "70-80. Suggest: gentle follow-up, ask if any questions before "
        "securing.\n"
        "  - Negotiating actively, engaged, dates fixed → 65-80.\n\n"
        "  DROP EVERYTHING (KEEP_OPEN, score 85-100):\n"
        "  - Strong booking signals: dates fixed, group set, asked for "
        "payment link, ready to pay → 85-100.\n"
        "  - Booking date is within 3 days AND customer engaged → "
        "boost score to 85+ regardless of other factors. Date urgency "
        "trumps almost everything.\n"
        "  - Large-yacht booking (100+ ft tier: AK Royalty, Mila, Cante, "
        "Luna, Notorious, Sapphire, Athena, Skyfall, Tatti, Odysea, "
        "Royal Mirage, Asya, Finesse, Carina, Haigan) within 14 days "
        "AND engaged → 85-100. Highest revenue per charter; operator "
        "should prioritize.\n\n"
